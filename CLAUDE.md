...
| Decision | Choice | Why |
|---|---|---|
| **Multi-district foundation (district-ready, not multi-district yet)** ⚠ **V2 + `sql/`** | A `district` column (default `'KUNAK'`) on `hydrants`, `hydrant_records`, `jadual_pemeriksaan`, `profiles`; a new `can_write(target)` that scopes **writes** to the officer's own district while **reads stay global** (mutual aid); every V2 query filtered by the officer's `profiles.district`, every write stamped with it. **No visible district selector** and **no second district onboarded** | Owner's product call, 2026-08-15: build the ~2-day foundation now as insurance, changing **nothing an officer sees** (all data is KUNAK), so any future district becomes a data-entry job rather than an engineering project — and keeping each phone's load **flat** (§7.2(5) in `docs/PRD.md`). Do NOT fork per district or per-district files (PRD §7.1 — the 7-commit drift is why). `can_write` follows the `is_admin` rule to the letter: `authenticated` **keeps EXECUTE** (an RLS policy runs as the calling role — §5), revoke from `public,anon` only; hardening re-asserts it. The signed-row guard is preserved **alongside** the district scope, never replaced. The officer's district comes from `auth.js` and **fails closed to KUNAK** so a read error can't widen scope. Visible selector deferred to district #2 (a one-option selector is noise). V1 is the frozen rollback and is untouched — safe **while single-district**. Full design + status in `docs/PRD.md` §7.3; security property guarded by `tests/v2-record-sync.js` T9 (mutation-checked), read-scope by T10. **Full activation — the `upper_admin` (district-manager) role tier, an in-app account-provisioning panel with district-scoped `profiles` RLS + self-escalation guards, and the `All › Kunak › Tawau` selector — is DESIGNED AND DEFERRED in `docs/PRD.md` §7.4; build it when district #2 actually onboards, not before** |
...
| Chart background stage | **Removed, then RESTORED 2026-08-13** as `#dashView .dsec` — the same `#121419` panel the tables use | Removing it was right when the dashboard sat on a flat ground: the stage added nothing. The premise changed. The chart palette and the green/blue/red figure ink were all measured against `#121419`, and the artwork now behind the dashboard breaks every one of those measurements — the navy segment and the *Belum diperiksa* label went first, dark ink on a dark busy ground. User asked for the donut's colours to be visible; a panel restores the contrast the palette was designed for, and the artwork still reads around and between the panels |
...
| **Liquid glass, with CAF0F8 as an ACCENT** ⚠ **V2 ONLY** | The login card, the **active tab** (was solid red), and the card material on all three tabs — `#dashView .dsec`/`.dcard`, `#profileView .pvcard`, `.panel` (the map's Registry card). Surfaces stay **dark**; `#CAF0F8` is the rim light, the sheen, the active tab's label and the Sign In button | User's call, 2026-08-13, from the `ui-ux-pro-max` skill. **CAF0F8 is an accent and never a surface, and that was the decision the numbers forced.** Its relative luminance is **0.82** — near white. As a background every text colour in the app fails against it: body text **1.04:1**, the wordmark **2.31:1**, the dashboard's green/blue/red figure ink **1.44 / 2.10 / 2.28:1**; only near-black passes. The other way round it is excellent — **14.8:1** on a card, **16.3:1** on the gate. Shown those figures the user chose accent-on-dark, which is why nothing measured had to be re-derived. **`--glass-bg` is `rgba(18,20,25,.88)` and 18,20,25 IS `#121419`** — the exact panel colour the figure ink was measured on. Re-measured through the glass from rendered pixels: **10.57 / 7.25 / 6.66:1**, against the documented 10.6 / 7.3 / 6.7. Raising the transparency is not a free "more glass": it lifts the panel toward the artwork's gold lines and drags every figure with it. The skill's own `liquid-glass` entry is the source for the rest — *apply sparingly to navigation and controls*, *keep content on a separate layer*, *apply colour judiciously*, `--tint: semantic`, and **test reduced transparency**, which is why `@media (prefers-reduced-transparency: reduce)` returns every surface to its opaque base. **The active tab lost its red fill and needed four signals to replace it** — brighter glass, a CAF0F8 rim, a CAF0F8 label (**10.24:1**) and a lift. The rim is an `inset` shadow, not a border: `.tabb` is `border:none`, so a border on `.on` alone would shift all three tabs on every view change. `aria-selected` still carries the state for assistive tech, so it is not colour-alone. **The login card's tint was NOT touched.** `--color-glass3d` is the measured value T9 holds the floor on — the last time it moved the wordmark fell to 1.03:1 — so the accent went on the edges and the button instead, and T9's seven colours still pass. The crest went **52px → 88px**. The divergence from V1's solid `.dcard` is pinned in `v2-dashboard-css.js`, which asserts **both** V2's new value and V1's unchanged one |
| **Header: one row, no Live pip, no role badge** ⚠ **V2 ONLY** | Awam / Swasta / Tambah Pili / DATE / TIME / Sign out share **one line down to 1024px**. The green **Live** pip and the **Admin/Viewer** badge are removed; the role is read on **Profil** (`#pvRole`) | User's call, 2026-08-13. Below 1280px that row wrapped and the header went **124px → 172px**, costing ~48px of map on the tab where the map is the point. The pip never reported anything an officer acts on, and the badge duplicated something the Profil tab already says one tap away — so both were width being spent on decoration. Removing them reached 1100px; the rest is spacing in **`map.css`** (`.hrow` gap 16→12, `.hright` 10→8, the `.clock` inset and the `.divider` gone, header padding 28→22), which is where `header{}` and `.hrow{}` actually win — shell.css declares both and is imported first (§4.27). **1024px is the floor and it is deliberate**: the row needs 1020px of content and the brand and pills are not negotiable, so a portrait tablet still wraps. Guarded by `v2-shell.js` **T9**, which measures the row's height against its tallest child — there is no class or computed style that says "this wrapped", since `flex-wrap:wrap` makes wrapping the normal outcome (§4.26 again). Its first version compared children's `top` values and reported 3 rows at every width, because `align-items:center` legitimately gives a pill and the brand block different y. `#roleBadge`/`#roleTxt` are waived in `parity-waivers.json` as **relocated, not dropped** |
| **A viewer cannot edit the Kad Rekod** ⚠ **V2 ONLY (a parity gap, now closed)** | Every input `disabled`, no **Save**, no **Sign**, a **Read-only** chip in its place. **Print and Close stay.** The Profil tab shows a viewer no signature card at all | V1 has done this since the beginning in `applyFormReadOnly()`. **V2 declared the `isAdmin` prop in Phase 5 and never used it**, so a viewer could type into a legal record's card and press Save — RLS refuses the write, so the typing is parked locally and lost rather than filed, which is §4.10 wearing a friendlier face. Measured on the shipped build: **324 writable inputs and 37 live Sign buttons** for a viewer. Found by reading the prop; `v2-parity-surface` was green throughout because the controls all EXIST — its own header says it catches absence, not wrongness, and this is the fourth defect of that kind after §4.22 and §4.23. **Hiding controls is courtesy, never the control** — `hydrant_records` RLS and the `protect_signed_rows` trigger are what actually refuse a viewer, and nothing about them changed. Print stays because reading and filing a record is exactly what a viewer is for. The signature card goes whole rather than button-by-button: a viewer has no signature to store and cannot sign a row, so the panel could only ever explain a capability they would never use. Guarded by `v2-app-live.js` **T21b**, which asserts EVERY input rather than a sample (the header fields and the table cells are separate bindings) and pairs each viewer assertion with the same card as an admin, or it would pass on a card that renders nothing |
| **The app artwork** ⚠ **V2 ONLY** | **One image, one layer, all three tabs.** `percut19_generated.jpg` (dark navy waves, gold and red line accents) on `body`, for **Peta Pili, Dashboard and Profil** alike. **The 50 survives on the LOGIN GATE and nowhere else** (`#authGate::before`, still `mix-blend-mode: multiply`, still no `filter`). The header carries a measured `rgba(8,14,24,.45)` scrim | User's call, 2026-08-13: use this behind the three tabs, remove the old background, remove the 50 from them. It replaced **two** mechanisms — the circuit board on `body`, and a pair of `.app` pseudo-elements that gave Dashboard and Profil a rotated copy of it plus the 50, toggled by `.bg-alt`. All of that is gone, `app-bg.png` with it, so there is no per-tab ground left to keep in step. **The scrim is the only part that is not taste.** The header is the one place text sits directly on artwork — everything else paints its own panel — and this artwork's gold lines are the brightest thing on the page. Against the brightest pixel it puts in that band the wordmark measured **2.31:1**, under the 3:1 large-text floor; at `.45` it is **3.47:1** and every other header colour clears 4.5:1. Light enough that the waves still read through. **It goes in `map.css`, not `shell.css`** — `header{}` is declared in both and `map.css` is imported last, so a copy written in `shell.css` would apply to nothing (§4.27). `.clock .u` (the DATE/TIME micro-labels) still measures **2.65:1** and is deliberately left: it is `rgba(255,255,255,.35)`, fails on V1's flat ground too, and no scrim reaches 4.5 — changing it is a decision about V1's design, not a side effect of a background swap. Pinned by `tests/v2-app-live.js` **T9b**, which samples the artwork's brightest pixel with the header's own scrim left in place, and asserts the scrim's alpha so a build that drops it cannot pass. **Multiply and the no-`filter` rule still apply to the gate** — the 50 is white-backed, and a `filter` on the blended element isolates it and switches the blend off (§4.28) |
| **Login gate: background only, then liquid glass** ⚠ **V2 ONLY** | The smoke photograph is replaced by a near-white page with the 50th logo behind the card. The card keeps its structure and type but takes the **liquid-glass** material: deeper blur, a sheen, a stronger rim light, 30px corners, **pill fields**, and a **cream Sign In button** with a glow. Tint `--color-glass3d` is **navy** `rgba(14,25,41,.89)` | User's calls, 2026-08-10 and 08-13 — *change only the background*, then *just add liquid glass, maintain everything else*. The wordmark stays **mixed-case orange** and the crest stays: one product, one wordmark between sign-in and the app. **The tint is not free.** The card is GLASS, so it tints what is behind it: on the light page a low tint rendered it mid-grey (`rgb(152,152,152)`) and every text colour collapsed — the wordmark to **1.03:1**. **And it came out BROWN**, because heavy `saturate()` amplifies whatever is behind, and behind it is a sepia engraving on cream; the fix was saturation `3.6 → 1.5` **plus** a navy tint, not tint alone. If it ever looks brown again, check the saturation before the tint. Final: worst large **3.31**, worst normal **5.83**; cream button ink at **13.5:1**. T9 measures all seven against rendered pixels, so none of this can be nudged without the suite saying so |
...
| **Login gate: 0077B6 glass at 70%, amber contra, WHITE wordmark** ⚠ **V2 ONLY** ⚠ **supersedes the three gate rows above on the login card only** | The login card's liquid glass is now **`#0077B6` as the body tint** (`--color-glass3d: rgba(1,46,74,.70)` — the hue, deepened), at **70% opacity**. The **contra colour, amber `#F5A524`**, carries the rim, the outer glow, the Sign In button (near-black ink) and the focus ring — the body is blue, the trim is warm. The **wordmark and BBP KUNAK are WHITE**, not orange | User's calls, 2026-08-14, iterating with the `ui-ux-pro-max` skill: 0077B6 for the glass, a contra colour for the accents, transparency at 70. **The white wordmark is forced, not a preference.** On the 70% 0077B6 panel the orange wordmark measures **2.13:1** and amber **2.1:1** — both fail; white is the only mark that clears (≈6:1). So the brand orange moved to the button/rim (its true complement is amber ~21°, which *is* the brand orange) and the wordmark went white. **70% is the FLOOR.** A swept measurement showed the wordmark is the *sole* blocker: everything else survives to ~74%, and with the orange wordmark kept the floor was 90%. Below 70% the footer/eye go too. **Semi-transparent body text was raised** — label/tagline/footer/eye to ~.95 — because the lighter panel sinks composited whites; the tagline and footer alphas live as **inline styles in `AuthGate.vue`**, which override the stylesheet, so they were bumped there. All seven gate colours were **re-measured on the assembled build** by `v2-app-live.js`'s gate check (wordmark 5.98, labels/footer/eye 4.79–5.49, button 8.49); `v2-shell.js` now asserts the wordmark computes to `rgb(255,255,255)` so a drift back to a failing colour is caught. **The global `--accent` (CAF0F8) is untouched** — it still drives the active tab and the dashboard/registry cards; only the gate's own button and rim moved to amber. `saturate()` stays 1.5 and the tint stays blue, so the sepia watermark cannot dye it brown (§4.28 family) |
...
27. **The login gate was defined twice, and the copy that won was in the record
    card's stylesheet.** Found 2026-08-10 while restyling the login wordmark.

    All **18** auth selectors — `#authGate`, `.authbox`, `.authbox h2`,
    `.authbtn`, `.authlogo` and the rest — existed byte-for-byte identically in
    **both** `shell.css` and `kad-rekod.css`. Nothing looked wrong, because the
    two copies agreed.

    The hazard is what happens the moment they stop agreeing. `main.js` imports
    `kad-rekod.css` **last**, deliberately, so the print rules win — which means
    that copy also won on the login gate. **A change made in `shell.css` alone
    would have applied to nothing**, silently, overridden by an identical rule
    in a stylesheet about a completely different screen. Duplicate removed;
    `shell.css` is now the single definition, with a note left behind saying so.

    Two things worth carrying:

    - **Check which copy wins before editing, not after.** The tell was cheap —
      grep the selector across `v2/src/styles/` and count the files. I only
      looked because the wordmark change touched `.authbox h2`, and would have
      spent the next round debugging why an edit "did nothing" otherwise. Same
      family as §5's *verify an edit landed*.
    - **Import order that is load-bearing for ONE purpose silently applies to
      everything else in the file.** `kad-rekod.css` is last so the print rules
...
28. **A `filter` silently switched off `mix-blend-mode`, and I read the symptom
    ...
    The artwork is **white-backed**, so it only works as a watermark under
    `mix-blend-mode: multiply` — white multiplies away and the engraved lines
    darken what is beneath. To make the engraving read on a dark ground I added
    `filter: contrast(1.9) brightness(.62)` to the same element.

    **A filter isolates the element**, and an isolated element's
    `mix-blend-mode` no longer blends with its backdrop. The blend stopped
    working entirely and the artwork's white plate painted as a plain light
    rectangle — behind the crest on the header, and around the 50 on Profil.

    Three things worth carrying:

    - **I diagnosed it as "too subtle" and made it stronger.** The rectangle was
      in a screenshot I had already looked at; I read a washed-out patch as
      "needs more filter" and pushed harder on the very thing causing it. The
      fix was to REMOVE the adjustment, not increase it.
    - **Proven by deletion, not by argument.** Taking the filter off and
      re-rendering made the rectangle vanish in one step. Whether a filter
      isolates is a genuinely subtle spec question; the experiment took a
      minute and was not ambiguous.
    - **If a blended layer needs more weight it must come from the ASSET or from
...
- **Wrote a staleness guard that was always "stale", and nothing errored.**
  The Sign popup fetches the officer's Profile signature asynchronously, so the
  result is discarded if they moved on: `if (signing.value === e)`. `ref()`
  **deep-converts an object to a reactive proxy**, so `signing.value` is never
  `===` the object assigned to it — the comparison was false every time and
  every fetch was thrown away. The visible effect was the popup offering to add
  a signature the officer already had, which looks exactly like "they have no
  signature". No error, no warning, and the Profile tab rendered the same image
  correctly two feet away, so every other check said the plumbing worked. Fixed
  with a monotonic integer token: **a primitive cannot be proxied.** Worth
  remembering as a family, not a one-off — `ref(obj).value !== obj`,
  `toRaw()` exists for this, and any identity comparison against a ref's
  contents is suspect.
