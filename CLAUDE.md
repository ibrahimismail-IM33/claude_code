...
| **Login gate: background only** ⚠ **V2 ONLY** | The smoke photograph is replaced by a near-white page with the 50th-anniversary logo watermarked behind the card. **The card, its fields and its button are untouched.** One value changed with it: `--color-glass3d` `.42` → `.88` | User's call, 2026-08-10 — *change only the background*. The one forced change is not cosmetic: **the card is GLASS**, so it tints whatever is behind it. On the old dark photograph `.42` produced a dark panel; on the light page it produced a **mid-grey one, measured `rgb(152,152,152)`**, and every light text colour on it collapsed — the wordmark to **1.03:1**, the footer note to **1.44:1**, unreadable on the first screen an officer sees. Raising the card's own tint keeps the glass, the shape, the blur and every text colour as they were and is the smallest fix. Three semi-transparent text values were also raised (`.45`→`.72`, `.3`→`.68`, `.55`→`.80`): those failed on the **old** background too, so the measurement found a pre-existing defect rather than one the new ground caused |
