...
| **The 50th-anniversary artwork** ⚠ **V2 ONLY** | `v2/public/logo-50.png` (the 50, **engraved style, WHITE-BACKED — no transparency**) and `v2/public/app-bg.png` (the circuit board, 1920×1120). **Login** = near-white ground + the 50 watermarked large. **App shell** = the circuit board on `body`, the 50 on `body::before`, behind the header, dashboard and Profil. Both watermarks use **`mix-blend-mode: multiply`** | User-supplied, replaced 2026-08-13. **Multiply is load-bearing, not decoration**: the asset has no alpha, so painted normally it is a white plate — invisible on the light login page, a white block over the dark app. Multiply drops the white and lets only the engraving darken what is beneath, so **one file and one technique serve both surfaces**. On the dark ground it also needs `filter:contrast(1.9) brightness(.62)` first — grey linework multiplied onto an already-dark background darkens by almost nothing and the first attempt was simply invisible. The circuit is positioned **`right center`**: its traces all enter from the right and `cover` crops width, so `center` throws away the part that was asked for — it needed **positioning, not rotation**. **Where it is actually visible:** the header strip, the dashboard and Profil. The map covers everything else and `.leaflet-container` paints its own ground — nothing is layered over it, because a background on a Leaflet ancestor is one edit away from a transform on one (§4.16/§4.17). Root-absolute urls, gated by `verify-bundle.js`; T9 asserts both load **and** that the watermark makes the page darker, not lighter |
