...
| **The 50th-anniversary artwork** ⚠ **V2 ONLY** | `v2/public/logo-50.png` (the 50 mark, transparent) and `v2/public/app-bg.jpg` (the circuit board). **Login** = near-white ground + the 50 watermarked large (background only — the card is untouched). **App shell** = the circuit board on `body`, with the 50 as a desaturated ghost on `body::before`, behind the header, dashboard and Profil. `login-bg.jpg` deleted | User-supplied, 2026-08-10. Two different grounds is the mockups' own split, not a compromise. **Each watermark needs its OWN layer** because a `background-image` layer cannot carry an `opacity` or a `filter` — at full colour the red 50 read as *content* on the dashboard and sat over a section heading. On the login it is **below** the card, not behind it: the card is 392px wide and owns the middle of a portrait phone, so a centred watermark is invisible at any size. The shell background is on `body`, never on a Leaflet ancestor — `.leaflet-container` paints its own ground and §4.16/§4.17 are what happens when Leaflet's geometry stops matching the page. Root-absolute urls, gated by `verify-bundle.js` and asserted **loaded** by T9 |
