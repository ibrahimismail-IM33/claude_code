...
| **The 50th-anniversary artwork** ⚠ **V2 ONLY** | `logo-50.png` (the 50, **engraved, WHITE-BACKED — no transparency**) and `app-bg.png` (circuit board). **Login** = near-white ground + the 50 large. **Peta Pili** = the circuit **as drawn** (horizontal traces) on `body`. **Dashboard / Profil** = the same circuit **rotated 90°** to vertical traces and pushed further back, on `.app::before`, with the 50 upright at half-page on `.app::after`; App.vue toggles `.bg-alt`. Every watermark uses **`mix-blend-mode: multiply`** and **no `filter`** | User-supplied, replaced 2026-08-13; per-surface treatment their call. **Multiply is load-bearing**: the asset has no alpha, so painted normally it is a white plate. **A `filter` on the same element kills the blend** — it isolates it, and the plate paints as a light rectangle (§4.28). Weight must come from the asset or `opacity`, never a filter. **Only the CIRCUIT rotates**; the 50 stays upright — it contains words, and a quarter-turned wordmark is a mistake, not a watermark. They were one shared rule briefly and the 50 came out sideways. The rotation is on a **pseudo-element, never on `.app`**: a transform on a Leaflet ancestor makes it the containing block for the map's panes (§4.16/§4.17). 100vh × 100vw before a 90° turn is 100vw × 100vh after, so the layer covers any aspect ratio |
